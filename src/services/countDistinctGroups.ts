...
/**
 * Counts distinct logical groups (role_id, location_id, org_unit_id) for targeted positions.
 *
...
export function countDistinctGroups(params: {
  ...
  for (const positionId of targetPositionIds) {
    ...
    groups.add(`${occupant.role_id}__${occupant.location_id}__${occupant.org_unit_id}`);
  }
