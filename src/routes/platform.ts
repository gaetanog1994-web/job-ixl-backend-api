...
platformRouter.get("/companies/:companyId/perimeters", async (req, res, next) => {
    try {
        ...
        if (!access) return res.status(401).json({ ok: false, error: "Unauthorized" });
        const companyId = req.params.companyId;
...
platformRouter.get("/companies/:companyId/super-admins", requireOwner, async (req, res, next) => {
    try {
        ...
        return res.json({ ok: true, super_admins: rows, correlationId: (req as any).correlationId ?? null });
    } catch (error) {
        next(error);
    }
});
...
platformRouter.post("/companies/:companyId/perimeters", requireCompanyAdmin, async (req, res, next) => {
    ...
    try {
        ...
        await pool.query(
            `insert into app_config (singleton, max_applications, company_id, perimeter_id)
             values (true, 3, $1, $2)
             on conflict (company_id, perimeter_id, singleton) do nothing`,
            [companyId, perimeter.id]
        );
...
platformRouter.get("/companies/:companyId/perimeters/:perimeterId/admins", requireCompanyAdmin, async (req, res, next) => {
    try {
        ...
        return res.json({ ok: true, admins: rows, correlationId: (req as any).correlationId ?? null });
    } catch (error) {
        next(error);
    }
});
