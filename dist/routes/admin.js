...
adminRouter.post("/config/max-applications", async (req, res) => {
    ...
    try {
        ...
        const out = await withTx(async (client) => {
            const oldRow = await client.query(`select max_applications from app_config where singleton = true limit 1`);
            const oldMax = oldRow.rows?.[0]?.max_applications ?? null;
            await client.query(`update app_config set max_applications = $1 where singleton = true`, [newMax]);
...
adminRouter.delete("/test-scenarios/:id", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const delApps = await client.query(`delete from test_scenario_applications where scenario_id = $1`, [scenarioId]);
            const delScenario = await client.query(`delete from test_scenarios where id = $1`, [scenarioId]);
            return {
                scenarioId,
...
adminRouter.delete("/test-scenarios/:id/applications", async (req, res, next) => {
    try {
        ...
        const correlationId = req.correlationId;
        const scenarioId = req.params.id;
        const del = await pool.query(`delete from test_scenario_applications where scenario_id = $1`, [scenarioId]);
...
adminRouter.post("/test-scenarios", async (req, res, next) => {
    try {
        ...
        if (!name)
            return res.status(400).json({ ok: false, error: "missing name", correlationId });
        const { rows } = await pool.query(`insert into test_scenarios (name) values ($1) returning id, name`, [name]);
