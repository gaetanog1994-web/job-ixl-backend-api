...
import { loadCampaignLifecycle, getCampaignStatus, openReservations, closeReservations, openCampaign, closeCampaign, } from "../services/campaignLifecycle.js";
...
const ENABLE_HARNESS_ENDPOINTS = !isProduction &&
 ...
 */
function harnessOnly(req, res) {
    if (!ENABLE_HARNESS_ENDPOINTS) {
        res.status(403).json({
            ok: false,
            error: "HARNESS_DISABLED",
            message: "Harness endpoints are disabled for this environment. Set ENABLE_HARNESS_ENDPOINTS=true only in controlled dev/staging.",
        });
 ...
 */
export async function rebalanceApplicationsAfterMaxChange(client, newMax, companyId, perimeterId) {
    const applicationsRes = await client.query(`SELECT id, user_id, priority, created_at
         FROM applications
         WHERE company_id = $1
    ...
    if (plan.deletedIds.length > 0) {
        const delRes = await client.query(`DELETE FROM applications
             WHERE company_id = $1
               AND perimeter_id = $2
               AND id = ANY($3::uuid[])`, [companyId, perimeterId, plan.deletedIds]);
    ...
    if (plan.updates.length > 0) {
        ...
        const updRes = await client.query(`WITH data AS (
                SELECT *
            ...
            WHERE a.id = d.id
              ...
              AND a.priority IS DISTINCT FROM d.priority`, [ids, priorities, companyId, perimeterId]);
        prioritiesUpdated = updRes.rowCount ?? 0;
    }
    // Recalculate application_count for users who still have applications
    await client.query(`UPDATE users u
         ...
         WHERE u.id = x.user_id
           AND u.company_id = $1
           AND coalesce(u.perimeter_id, u.home_perimeter_id) = $2`, [companyId, perimeterId]);
...
function parseFixedLocation(value) {
    const normalized = value.trim().toLowerCase();
    if (!normalized)
        return false;
    if (normalized === "sì" || normalized === "si")
        return true;
    if (normalized === "no")
        return false;
    return null;
 ...
 */
adminRouter.post("/test-scenarios/:id/initialize", async (req, res) => {
    const r = req;
    ...
    try {
        const result = await withTx(async (client) => {
            ...
            const insertedApplications = await client.query(applicationsHasCampaignId
                ? `
            ...
            const reservedUsersRes = await client.query(`
        ...
        const message = String(e?.message ?? e);
        await audit("scenario_initialize", r.user.id, { scenarioId }, { error: message }, correlationId);
...
adminRouter.post("/users/invite", async (req, res) => {
    ...
    try {
        ...
        const normalizedAccessRole = access_role === "admin" || access_role === "admin_user" ? access_role : "user";
        ...
        await audit("admin_invite_user", req?.user?.id ?? "unknown", { email: req.body?.email ?? null }, { error: String(e?.message ?? e) }, correlationId);
 ...
 */
adminRouter.post("/config/max-applications", async (req, res) => {
    const r = req;
    const correlationId = req.correlationId;
    try {
        const { companyId, perimeterId } = getTenantScope(req);
        ...
        const out = await withTx(async (client) => {
            const oldRow = await client.query(`select max_applications from app_config where singleton = true and company_id = $1 and perimeter_id = $2 limit 1`, [companyId, perimeterId]);
            const oldMax = oldRow.rows?.[0]?.max_applications ?? null;
            await client.query(`update app_config set max_applications = $1 where singleton = true and company_id = $2 and perimeter_id = $3`, [newMax, companyId, perimeterId]);
            ...
            const rebalanceResult = await rebalanceApplicationsAfterMaxChange(client, newMax, companyId, perimeterId);
            return {
                oldMax,
                newMax,
                rebalance: {
        ...
        invalidateMapCache();
        await audit("config_update_max_applications", r.user.id, { newMax }, out, correlationId);
        return res.status(200).json({ ok: true, out, correlationId });
    }
    catch (e) {
        await audit("config_update_max_applications", req?.user?.id ?? "unknown", { body: req.body ?? {} }, { error: String(e?.message ?? e) }, req.correlationId);
        return res.status(500).json({
...
adminRouter.post("/users/reset-active", async (req, res) => {
    ...
    try {
        ...
        await audit("users_reset_active", r.user.id, {}, { error: String(e?.message ?? e) }, correlationId);
        return res.status(500).json({
...
async function readLifecycleSnapshot(client, companyId, perimeterId) {
 ...
 */
adminRouter.post("/reservations/open", requireOperationalPerimeterAdmin, async (req, res) => {
    const r = req;
    const correlationId = req.correlationId;
    try {
        const { companyId, perimeterId } = getTenantScope(req);
        ...
        invalidateMapCache();
        await audit("admin_open_reservations", r.user.id, { company_id: companyId, perimeter_id: perimeterId }, out.snapshot, correlationId, { companyId, perimeterId });
 ...
 */
adminRouter.post("/reservations/close", requireOperationalPerimeterAdmin, async (req, res) => {
    const r = req;
    const correlationId = req.correlationId;
    try {
        const { companyId, perimeterId } = getTenantScope(req);
        ...
        invalidateMapCache();
        await audit("admin_close_reservations", r.user.id, { company_id: companyId, perimeter_id: perimeterId }, out.snapshot, correlationId, { companyId, perimeterId });
        return res.json({ ok: true, ...(out.snapshot ?? {}), correlationId });
    }
    catch (e) {
        return res.status(500).json({
 ...
 */
adminRouter.post("/campaign/open", requireOperationalPerimeterAdmin, async (req, res) => {
    const r = req;
    const correlationId = req.correlationId;
    try {
        const { companyId, perimeterId } = getTenantScope(req);
        ...
        invalidateMapCache();
        await audit("admin_open_campaign", r.user.id, { company_id: companyId, perimeter_id: perimeterId }, { ...(out.snapshot ?? {}), usersUpdated: out.usersUpdated }, correlationId, { companyId, perimeterId });
        return res.json({ ok: true, ...(out.snapshot ?? {}), correlationId });
    }
    catch (e) {
        return res.status(500).json({
 ...
 */
adminRouter.post("/campaign/close", requireOperationalPerimeterAdmin, async (req, res) => {
    const r = req;
    const correlationId = req.correlationId;
    try {
        const { companyId, perimeterId } = getTenantScope(req);
        ...
        invalidateMapCache();
        await audit("admin_close_campaign", r.user.id, { company_id: companyId, perimeter_id: perimeterId }, {
            ...(out.snapshot ?? {}),
            usersReset: out.usersReset,
            applicationsDeleted: out.applicationsDeleted,
            testScenarioUsersReset: out.testScenarioUsersReset,
        }, correlationId, { companyId, perimeterId });
        return res.json({ ok: true, ...(out.snapshot ?? {}), correlationId });
    }
    catch (e) {
        return res.status(500).json({
...
adminRouter.get("/candidatures", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        const { companyId, perimeterId } = getTenantScope(req);
        if (!companyId || !perimeterId) {
            return res.status(400).json({ ok: false, error: "PERIMETER_CONTEXT_REQUIRED", correlationId: req.correlationId ?? null });
        }
 ...
 */
adminRouter.get("/users/import-template", requireOperationalPerimeterAdmin, async (req, res) => {
    try {
        const { companyId, perimeterId } = getTenantScope(req);
        if (!companyId || !perimeterId) {
            return res.status(400).json({
        ...
        const [rolesRes, locationsRes] = await Promise.all([
            pool.query(`select name from roles order by name asc`),
        ...
        res.setHeader("Content-Disposition", "attachment; filename=\"template_importazione_utenti.xlsx\"");
 ...
 */
adminRouter.post("/users/import", requireOperationalPerimeterAdmin, uploadExcel.single("file"), async (req, res) => {
    const r = req;
    const correlationId = req.correlationId;
    const file = req.file;
    try {
        ...
        await audit("bulk_user_import", r.user.id, { companyId, perimeterId }, { total, imported, errors: errors.length }, correlationId);
        return res.status(200).json({
            total,
            imported,
            errors,
...
adminRouter.delete("/users/:id", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        ...
        const { companyId, perimeterId } = getTenantScope(req);
        await pool.query(`delete from applications where user_id = $1 and company_id = $2 and perimeter_id = $3`, [userId, companyId, perimeterId]);
        const del = await pool.query(`delete from users where id = $1 and company_id = $2 and coalesce(perimeter_id, home_perimeter_id) = $3`, [userId, companyId, perimeterId]);
        invalidateMapCache();
...
adminRouter.patch("/users/:id", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        ...
        const correlationId = req.correlationId;
        const { companyId, perimeterId } = getTenantScope(req);
        const { availability_status, first_name, last_name, email, role_id, org_unit_id, location_id, fixed_location, access_role, } = req.body ?? {};
        ...
        if (org_unit_id !== undefined)
            push("org_unit_id", org_unit_id || null);
        if (location_id !== undefined)
            push("location_id", location_id || null);
        if (fixed_location !== undefined)
            push("fixed_location", !!fixed_location);
        if (role_id !== undefined)
            push("role_id", role_id || null);
        ...
        const userFromDb = rows?.[0]
            ...
            : (await pool.query(`
                ...
                where id = $1
                  ...
                  and coalesce(perimeter_id, home_perimeter_id) = $3
                limit 1
                `, [userId, companyId, perimeterId])).rows?.[0] ?? null;
...
adminRouter.get("/positions", async (req, res, next) => {
    try {
      ...
      limit 1000
    `, [companyId, perimeterId]);
        return res.json({ ok: true, positions: rows, correlationId: req.correlationId ?? null });
    }
    catch (e) {
        next(e);
    }
...
adminRouter.get("/locations", async (req, res, next) => {
    try {
      ...
      limit 2000
    `);
        return res.json({ ok: true, locations: rows, correlationId: req.correlationId ?? null });
    }
    catch (e) {
        next(e);
    }
...
adminRouter.get("/roles", async (req, res, next) => {
    try {
        ...
        const rolesRes = await pool.query(hasRoleTenantScope
            ? `
            ...
            : `
                  ...
                  order by sr.name asc
                  `, [companyId, perimeterId]);
        const compatRes = await pool.query(hasRoleTenantScope
            ? `
                  with scoped_roles as (
                    select r.id
                    from roles r
            ...
            : `
                  ...
                  order by cr.name asc
                  `, [companyId, perimeterId]);
        const compatByRoleId = new Map();
        for (const row of compatRes.rows) {
            const roleId = String(row.role_id);
            if (!compatByRoleId.has(roleId))
                compatByRoleId.set(roleId, []);
...
adminRouter.delete("/roles/:id", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        ...
        const out = await withTx(async (client) => {
            await client.query(`delete from role_compatibilities where role_id = $1 or compatible_role_id = $1`, [roleId]);
...
adminRouter.post("/roles/:id/compatibility", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        ...
        if (!targetId)
            return res.status(400).json({ ok: false, error: "missing targetRoleId", correlationId });
        if (roleId === targetId) {
        ...
        const foundIds = new Set(scopedRoles.rows.map((row) => row.id));
        ...
        await audit("role_compatibility_add", r.user.id, { roleId, targetId, companyId, perimeterId }, out, correlationId);
...
adminRouter.delete("/roles/:id/compatibility/:targetId", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        ...
        if (!targetId)
            return res.status(400).json({ ok: false, error: "missing targetId", correlationId });
        const out = await withTx(async (client) => {
            const delDirect = await client.query(`delete from role_compatibilities where role_id = $1 and compatible_role_id = $2`, [roleId, targetId]);
            const delReverse = await client.query(`delete from role_compatibilities where role_id = $1 and compatible_role_id = $2`, [targetId, roleId]);
            return {
                removed: (delDirect.rowCount ?? 0) + (delReverse.rowCount ?? 0),
            };
        });
        await audit("role_compatibility_remove", r.user.id, { roleId, targetId, companyId, perimeterId }, out, correlationId);
        return res.json({ ok: true, ...out, correlationId });
    }
    catch (e) {
        next(e);
...
adminRouter.post("/org-units", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        ...
        let level = 1;
        if (parentId) {
            const parentRes = await pool.query(`select level from organizational_units where id = $1 and company_id = $2 and perimeter_id = $3 limit 1`, [parentId, companyId, perimeterId]);
            if (!parentRes.rows[0]) {
        ...
        await audit("org_unit_create", req.user?.id, { name, parentId, level, companyId, perimeterId }, rows[0], correlationId);
...
adminRouter.patch("/org-units/:id", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        ...
        const parentId = req.body?.parent_id !== undefined
            ...
            : undefined;
        const fields = ["name = $1"];
        const values = [name, orgUnitId, companyId, perimeterId];
        ...
        await audit("org_unit_update", req.user?.id, { orgUnitId, name, parentId }, rows[0], correlationId);
...
adminRouter.patch("/departments/:id", requireOperationalPerimeterAdmin, async (req, res, next) => {
    ...
    try {
        ...
        if (!name)
            return res.status(400).json({ ok: false, error: "missing name", correlationId });
        const { rows } = await pool.query(`update organizational_units set name = $1 where id = $2 and company_id = $3 and perimeter_id = $4 returning id, name, parent_id, level`, [name, orgUnitId, companyId, perimeterId]);
        if (!rows[0])
            return res.status(404).json({ ok: false, error: "ORG_UNIT_NOT_FOUND", correlationId });
        ...
        if (String(e?.code ?? "") === "23505") {
            return res.status(409).json({ ok: false, error: "ORG_UNIT_ALREADY_EXISTS", correlationId: req.correlationId ?? null });
        }
        next(e);
    }
});
adminRouter.delete("/org-units/:id", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        ...
        const childrenRes = await pool.query(`select count(*)::int as cnt from organizational_units where parent_id = $1 and company_id = $2 and perimeter_id = $3`, [orgUnitId, companyId, perimeterId]);
        ...
        const usersRes = await pool.query(`select count(*)::int as cnt from users where org_unit_id = $1 and company_id = $2 and coalesce(perimeter_id, home_perimeter_id) = $3`, [orgUnitId, companyId, perimeterId]);
        ...
        const del = await pool.query(`delete from organizational_units where id = $1 and company_id = $2 and perimeter_id = $3`, [orgUnitId, companyId, perimeterId]);
        if ((del.rowCount ?? 0) === 0) {
            return res.status(404).json({ ok: false, error: "ORG_UNIT_NOT_FOUND", correlationId });
        }
        await audit("org_unit_delete", req.user?.id, { orgUnitId, companyId, perimeterId }, { deleted: 1 }, correlationId);
        return res.json({ ok: true, deleted: del.rowCount ?? 0, correlationId });
    }
    catch (e) {
        next(e);
...
function registerManagerEntityRoutes(config) {
    ...
    adminRouter.post(`${resourcePath}/:id/assign`, requireOperationalPerimeterAdmin, async (req, res, next) => {
        try {
            ...
            const scopedUserIds = new Set(scopedUsers.rows.map((row) => row.id));
...
adminRouter.get("/test-scenarios", async (req, res, next) => {
    try {
      ...
      limit 500
    `, [companyId, perimeterId]);
        return res.json({ ok: true, scenarios: rows, correlationId: req.correlationId ?? null });
    }
    catch (e) {
        next(e);
    }
 ...
 */
adminRouter.patch("/test-scenarios/:id", async (req, res, next) => {
    try {
        const r = req;
        const correlationId = req.correlationId;
        const scenarioId = req.params.id;
 ...
 */
adminRouter.delete("/test-scenarios/:id", async (req, res) => {
    const r = req;
    const correlationId = req.correlationId;
    const scenarioId = req.params.id;
    const { companyId, perimeterId } = getTenantScope(req);
    try {
        const out = await withTx(async (client) => {
            const delApps = await client.query(`delete from test_scenario_applications where scenario_id = $1 and company_id = $2 and perimeter_id = $3`, [scenarioId, companyId, perimeterId]);
            const delScenario = await client.query(`delete from test_scenarios where id = $1 and company_id = $2 and perimeter_id = $3`, [scenarioId, companyId, perimeterId]);
            return {
                scenarioId,
        ...
        await audit("scenario_delete", r.user.id, { scenarioId }, { error: String(e?.message ?? e) }, correlationId);
 ...
 */
adminRouter.delete("/test-scenarios/:id/applications", async (req, res, next) => {
    try {
        const r = req;
        const correlationId = req.correlationId;
        const scenarioId = req.params.id;
        const { companyId, perimeterId } = getTenantScope(req);
        const del = await pool.query(`delete from test_scenario_applications where scenario_id = $1 and company_id = $2 and perimeter_id = $3`, [scenarioId, companyId, perimeterId]);
...
adminRouter.post("/locations", async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`insert into locations (name, latitude, longitude)
       values ($1, $2, $3)
       returning id, name, latitude, longitude`, [String(name), latitude ?? null, longitude ?? null]);
        invalidateMapCache();
        return res.status(201).json({ ok: true, location: rows[0], correlationId: req.correlationId ?? null });
    }
    catch (e) {
        next(e);
    }
...
adminRouter.patch("/locations/:id", async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(`update locations set ${setClauses.join(", ")} where id = $1 returning id, name, latitude, longitude`, params);
        if (!rows[0]) {
...
adminRouter.delete("/locations/:id", async (req, res, next) => {
    try {
        ...
        invalidateMapCache();
        return res.json({ ok: true, deleted: del.rowCount ?? 0, correlationId: req.correlationId ?? null });
    }
    catch (e) {
        next(e);
    }
 ...
 */
adminRouter.post("/test-scenarios", async (req, res, next) => {
    try {
        const r = req;
        const { companyId, perimeterId } = getTenantScope(req);
        const correlationId = req.correlationId;
 ...
 */
adminRouter.post("/test-scenarios/:id/applications", async (req, res, next) => {
    try {
        const r = req;
        const { companyId, perimeterId } = getTenantScope(req);
        const correlationId = req.correlationId;
...
adminRouter.get("/interlocking-scenarios/export.csv", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        ...
        res.setHeader("Content-Disposition", `attachment; filename="interlocking-scenarios-${perimeterId}.csv"`);
...
adminRouter.post("/interlocking-scenarios", requireOperationalPerimeterAdmin, async (req, res) => {
    ...
    const { companyId, perimeterId } = getTenantScope(req);
    try {
        const { campaign_id, scenario_code, generated_at, strategy, max_len, total_chains, unique_people, coverage, avg_length, max_length, avg_priority, build_nodes, build_relationships, chains_json, optimal_chains_json, } = req.body ?? {};
        ...
        const scenario = rows?.[0] ?? null;
        await audit("interlocking_scenario_create", r.user.id, {
            scenario_code: String(scenario_code),
            campaign_id: campaignId,
            company_id: companyId,
            perimeter_id: perimeterId,
        }, { scenario }, correlationId, { companyId, perimeterId });
        ...
        }, { error: String(e?.message ?? e) }, correlationId, { companyId, perimeterId });
        return res.status(500).json({
            ok: false,
            error: {
...
adminRouter.delete("/interlocking-scenarios", requireOperationalPerimeterAdmin, async (req, res) => {
    ...
    try {
        ...
        const campaignId = String(req.query?.campaign_id ?? req.body?.campaign_id ?? "").trim();
        let campaignFilterSql = "";
        const params = [ids, companyId, perimeterId];
        if (campaignId) {
        ...
        await audit("interlocking_scenario_delete_many", r.user.id, { ids, company_id: companyId, perimeter_id: perimeterId, campaign_id: campaignId || null }, out, correlationId, { companyId, perimeterId });
        return res.json({
            ok: true,
            out,
            correlationId,
        ...
        await audit("interlocking_scenario_delete_many", r.user.id, {
 ...
 */
adminRouter.get("/scenarios", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        const correlationId = req.correlationId;
        const campaignId = String(req.query?.campaign_id ?? "").trim();
        if (!campaignId) {
 ...
 */
adminRouter.post("/scenarios", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        const correlationId = req.correlationId;
        const r = req;
        const { companyId, perimeterId } = getTenantScope(req);
        ...
        await audit("interlocking_scenario_create", r.user.id, {
            ...
            company_id: companyId,
            perimeter_id: perimeterId,
        }, { scenario: inserted.rows?.[0] ?? null }, correlationId, { companyId, perimeterId });
...
adminRouter.get("/campaigns", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        const { companyId, perimeterId } = getTenantScope(req);
        if (!companyId || !perimeterId) {
            return res.status(400).json({ ok: false, error: "PERIMETER_CONTEXT_REQUIRED", correlationId: req.correlationId });
        }
        const { rows } = await pool.query(`SELECT id, status, reservations_opened_at, reservations_closed_at,
             ...
             LIMIT 100`, [companyId, perimeterId]);
        return res.json({ ok: true, campaigns: rows, correlationId: req.correlationId ?? null });
    }
    catch (e) {
        next(e);
    }
...
adminRouter.get("/campaigns/:campaignId", requireOperationalPerimeterAdmin, async (req, res, next) => {
    try {
        ...
        if (!campaignId) {
            return res.status(400).json({ ok: false, error: "CAMPAIGN_ID_REQUIRED", correlationId: req.correlationId });
        }
        const campaignRes = await pool.query(`SELECT id, status, reservations_opened_at, reservations_closed_at,
             ...
             LIMIT 1`, [campaignId, companyId, perimeterId]);
