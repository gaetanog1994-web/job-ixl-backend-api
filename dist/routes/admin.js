...
adminRouter.post("/users/:id/deactivate", async (req, res) => {
    ...
    try {
        ...
        await audit("user_deactivate", r.user.id, { userId }, { error: String(e?.message ?? e) }, correlationId);
...
adminRouter.get("/campaign-status", requireOperationalPerimeterAdmin, async (req, res) => {
    try {
        ...
        const { rows } = await pool.query(`select campaign_status from perimeters where id = $1 limit 1`, [perimeterId]);
        if (!rows.length) {
...
adminRouter.patch("/campaign-status", requireOperationalPerimeterAdmin, async (req, res) => {
    ...
    try {
        ...
        const { rows } = await pool.query(`update perimeters set campaign_status = $1 where id = $2 returning campaign_status`, [newStatus, perimeterId]);
        if (!rows.length) {
            return res.status(404).json({ ok: false, error: "PERIMETER_NOT_FOUND", correlationId });
        }
        invalidateMapCache();
...
adminRouter.post("/interlocking-scenarios", requireOperationalPerimeterAdmin, async (req, res) => {
    ...
    try {
        const { companyId, perimeterId } = getTenantScope(req);
        const { scenario_code, generated_at, strategy, max_len, total_chains, unique_people, coverage, avg_length, max_length, avg_priority, build_nodes, build_relationships, chains_json, optimal_chains_json, } = req.body ?? {};
        ...
        const scenario = rows?.[0] ?? null;
        await audit("interlocking_scenario_create", r.user.id, { scenario_code: String(scenario_code) }, { scenario }, correlationId);
